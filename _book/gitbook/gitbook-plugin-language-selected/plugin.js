function languagePickerGrid(gitbook, elem, maxColumns, langData) {
	...
	(elem.find('table').length == 0) && elem.append(table);
}
...
require(["gitbook", "jQuery"], function(gitbook, $) {
	gitbook.events.bind("start", function(e, config) {
		var opts = config["language-selected"];
		...
		$(function() {
			var DDMenu_selector = ".language-selected .dropdown-menu";
			$(document).on("click", ".language-selected .btn", function(e) {
				parseLanguages(gitbook, $(DDMenu_selector), opts["grid-columns"], function(elem) {
					elem.addClass("language-selected-grid");
